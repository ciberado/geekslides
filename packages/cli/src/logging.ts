/**
 * GeekSlides v2 — CLI logging (pino).
 *
 * Writes diagnostic logs to **stderr** so that stdout remains available for
 * user-facing CLI output (console.log).
 *
 * Configure via environment variables:
 *
 *   GEEKSLIDES_LOG=info                     # global level
 *   GEEKSLIDES_LOG=info,dev:debug,pdf:trace # per-namespace
...
const rootLogger: Logger = pino({
  ...
  ...(format !== 'json'
    ...
    : {}),
}, format === 'json' ? pino.destination(2) : undefined);
 ...
 * log.debug({ port: 5173 }, 'vite server started');
 * ```
 */

/* eslint-disable */
// @ts-nocheck
// Forked from pptx2html/src/process_pptx.js (MIT)
// Changes from upstream:
//   - Replaced global JSZip with direct import
//   - Replaced btoa()+loop with Buffer.from().toString('base64')
//   - Fixed tXml import path to local fork
//   - Guarded optional ppt/tableStyles.xml
'use strict'
...
function base64ArrayBuffer (arrayBuff) {
  return Buffer.from(arrayBuff).toString('base64')
}
...
export default function processPptx (setOnMessage = () => {}, postMessage) {
  ...
  async function processPPTX (data) {
    ...
    const _tableStylesFile = zip.file('ppt/tableStyles.xml')
    tableStyles = _tableStylesFile ? await readXmlFile(zip, 'ppt/tableStyles.xml') : null
